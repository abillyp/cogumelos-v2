...
function ModalEtiquetas({ exp, onFechar }: { exp: Experimento; onFechar: () => void }) {
  ...
  useEffect(() => {
    const expUrl = `${window.location.origin}/experimentos?exp=${encodeURIComponent(exp.codigo)}`
    setQrSrc(`/qr?url=${encodeURIComponent(expUrl)}`)
  }, [exp.codigo])
  ...
  function etiquetaHtml(num: number) {
    const qrAbsoluto = `${window.location.origin}/qr?url=${encodeURIComponent(`${window.location.origin}/experimentos?exp=${encodeURIComponent(exp.codigo)}`)}`
    return `
      <div style="width:220px;border:1.5px solid #222;border-radius:8px;overflow:hidden;font-family:monospace;background:#fff;display:inline-block;margin:4mm;vertical-align:top;">
        ...
        <div style="padding:8px 12px;background:#fff;display:flex;gap:8px;align-items:flex-start;">
          ...
          <img src="${qrAbsoluto}" width="48" height="48" style="flex-shrink:0;border-radius:3px;">
        </div>
